...
const Schedule = () => {
  ...
  useEffect(() => {
    if (
      !monthRange ||
      currentDate > monthRange.end ||
      currentDate < monthRange.begin
    ) {
  ...
  useEffect(() => {
    if (initialLoaded) applyLightFiltersAndOrder();
  }, [filters]);
  ...
  const applyLightFiltersAndOrder = (
    ...
    if (
      !(typeFilter?.length > 0) ||
      typeFilter.findIndex((typeObj) => typeObj.value === "campaigns") !== -1
    ) {
    ...
    if (
      ...
      typeFilter.findIndex(
        (typeObj) => typeObj.value !== "campaigns" && typeObj.value !== "tasks"
      ) !== -1
    ) {
    ...
    if (
      !(typeFilter?.length > 0) ||
      typeFilter.findIndex((typeObj) => typeObj.value === "tasks") !== -1
    ) {
  ...
  const mapCampaignMixed = (campaign) => ({
    ...campaign,
    itemType: "campaign",
    dropdownOpts: getDropdownOpts(campaign, "campaign"),
  });
  const mapProjectsMixed = (project) => ({
    ...project,
    itemType: "project",
    dropdownOpts: getDropdownOpts(project, "project"),
  });
  const mapTaskMixed = (task) => ({
    ...task,
    itemType: "task",
    dropdownOpts: getDropdownOpts(task, "task"),
  });

  const sortMixed = (mixedList) => {
    mixedList.sort((itemA, itemB) => {
  ...
  const findUsersFn = (filterMember) => (user) =>
    user.id === filterMember.value;

  const evalCampaignsWithFilters = (campaign) => {
    if (
      filters.campaign?.length > 0 &&
      filters.campaign.findIndex(
        (filterCampaign) => campaign.id === filterCampaign.value
      ) === -1
    )
      return false;
    if (
      ...
      filters.status.findIndex(
        (filterStatus) => campaign.status === filterStatus.value
      ) === -1
    )
      return false;
    if (
      filters.member?.length > 0 &&
      filters.member.findIndex((filterMember) =>
        campaign.users.find(findUsersFn(filterMember))
      ) === -1
    )
      return false;
  ...
  const evalProjectsWithFilters = (project) => {
    if (
      filters.status?.length > 0 &&
      filters.status.findIndex(
        (filterStatus) => project.status === filterStatus.value
      ) === -1
    )
      return false;
    if (
      ...
      filters.type.findIndex(
        (filterType) => project.type === filterType.value
      ) === -1
    )
      return false;
    if (
      filters.campaign?.length > 0 &&
      filters.campaign.findIndex(
        (filterCampaign) => project.campaignId === filterCampaign.value
      ) === -1
    )
      return false;
    if (
      filters.member?.length > 0 &&
      filters.member.findIndex((filterMember) =>
        project.users.find(findUsersFn(filterMember))
      ) === -1
    )
      return false;
  ...
  const evalTasksWithFilters = (task) => {
    if (
      filters.status?.length > 0 &&
      filters.status.findIndex(
        (filterStatus) => task.status === filterStatus.value
      ) === -1
    )
      return false;
    if (
      filters.campaign?.length > 0 &&
      filters.campaign.findIndex(
        (filterCampaign) => task.project?.campaignId === filterCampaign.value
      ) === -1
    )
      return false;
    if (
      filters.member?.length > 0 &&
      filters.member.findIndex((filterMember) =>
        task.users.find(findUsersFn(filterMember))
      ) === -1
    )
      return false;
  ...
  const updateItem = async (item, targetDate) => {
    try {
      ...
      const newDate = new Date(
        targetDate.setHours(existingDate.getHours(), existingDate.getSeconds())
      );
      const itemIndex = mixedList.findIndex(
      ...
      if (item.itemType === "campaign") {
        setMixedList(
          update(mixedList, { [itemIndex]: { endDate: { $set: newDate } } })
        );
        await campaignApi.updateCampaign(item.id, { endDate: newDate });
      } else if (item.itemType === "project") {
        setMixedList(
          update(mixedList, { [itemIndex]: { publishDate: { $set: newDate } } })
        );
    ...
    switch (type) {
      case "project":
        options.push({
          ...
          onClick: async () => {
            try {
              const { data } = await projectApi.createDuplicatedProject(
                item.id
              );
  ...
  return (
    <>
          ...
          <div className={styles.content}>
            ...
            {activeView === "list" && (
              <div className={styles.schedule}>
                <List mixedList={mixedList} currentDate={currentDate} />
              </div>
            )}
            {activeView === "week" && (
              <div className={styles.schedule}>
      ...
      {searchVisible && (
        <SearchOverlay closeOverlay={() => setSearchVisible(false)} />
      )}
    </>
  );
};
